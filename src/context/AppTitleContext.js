 ...
 * 
 */
const DEFAULT_APP_TITLE = "KOMNTR 😼";
...
let reducer = (state, action) => {
  switch (action.type) {
    case "default-title":
      return initialState;
    case "set-title":
      return {...state, titleColor: action.data.titleColor || "#624694", title: action.data.title };
    default:
        return { ...state };  
  }
};
...
function TitleContextProvider(props) {
  ...
  return (
    <TitleContext.Provider value={value}>{props.children}</TitleContext.Provider>
  );
}

let TitleContextConsumer = TitleContext.Consumer;

export { TitleContext, TitleContextProvider, TitleContextConsumer, DEFAULT_APP_COLOR, DEFAULT_APP_TITLE };
